...
| --- | --- |
| **记录日期** | 2026-05-19 |
| **Capgo 仓库提交** | 跟随你 checkout 的分支/commit（本仓库内一键脚本不再锁 ref；记录日期当时 `main` HEAD ≈ `955814dd39c66090958f41208a75a37c52f93e3c`） |
...
- 一键脚本只 pin **Supabase 上游**，`SUPABASE_DOCKER_REF` 默认与本表一致；跟踪上游 Supabase 最新可 `export SUPABASE_DOCKER_REF=master`（不推荐生产）。
- 本仓库（Capgo fork）**不**单独 pin 自身 ref —— 一键脚本就在本仓库内、随你 checkout 的分支 / commit 一起前进，且**必须**始终兼容当前 `SUPABASE_DOCKER_REF`。
...
4. **Capgo 代码**：要锁本仓库代码就在部署机上 `git checkout <sha>`（或 `SKIP_GIT_PULL=true`），验证后再切到新分支/tag；脚本不再用环境变量重复 pin。
